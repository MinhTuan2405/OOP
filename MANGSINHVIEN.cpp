...
Diem::Diem(int number_, double mark_[]){
    number = number_;
...
class SinhVien {
    ...
    public:
        ...
        SinhVien (string fullname_, string code_, Diem grade_){
            fullname = fullname_;
            code = code_;
            grade = grade_;
        }
...
class MangSinhVien{
    ...
    public:
        ...
        MangSinhVien (MangSinhVien* mangsinhvien){
            n = mangsinhvien -> n;
            sinhvien = mangsinhvien -> sinhvien;
        }
        ...
        friend std::istream& operator>> ( std::istream& in, MangSinhVien& mangsinhvien){
            for ( int i = 0; i < mangsinhvien.n; ++i){
                in >> *(mangsinhvien.sinhvien + i);
                in.ignore();
            }
            return in;
        }

        friend std::ostream& operator<< (std::ostream& out, MangSinhVien mangsinhvien){
            for ( int i = 0; i < mangsinhvien.n; ++i){
                out << *(mangsinhvien.sinhvien + i);
                out << '\n';
            }
            return out;
        }
        ...
        MangSinhVien operator= ( MangSinhVien mang2){
            n = mang2.n;
            sinhvien = mang2.sinhvien;
        }  

        double _AVERAGE_ (){
            double sum = 0;
            for ( int i = 0; i < n; ++i) sum += (sinhvien + i) -> get_average_mark();
            return sum / n;
        }

        int _MAX_AVERAGE_MARK_(){
            double max_average = _AVERAGE_();
            int index = 0;
            for ( int i = 0; i < n; ++i){
               if ( max_average < (sinhvien + i) -> get_average_mark()){
                    max_average = (sinhvien + i) -> get_average_mark();
                    index = i;
               }
            }
            return index;
